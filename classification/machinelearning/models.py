...
class RegressionModel(object):
    ...
    def train(self, dataset):
        ...
        for x, y in dataset.iterate_forever(batch_size):
            ...
            self.w1.update(g_w1, -learning_rate)
            self.w2.update(g_w2, -learning_rate)
            self.w3.update(g_w3, -learning_rate)

            self.b1.update(g_b1, -learning_rate)
            self.b2.update(g_b2, -learning_rate)
            self.b3.update(g_b3, -learning_rate)
...
class DigitClassificationModel(object):
    ...
    def __init__(self):
        # Initialize your model parameters here
        "*** YOUR CODE HERE ***"
        self.w1 = nn.Parameter(784, 784)
        self.b1 = nn.Parameter(1, 784)
        self.w2 = nn.Parameter(784, 300)
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        # Initialize your model parameters here
        "*** YOUR CODE HERE ***"
        #self.w1 = nn.Parameter(self.num_chars, self.num_chars)
        self.w1 = nn.Parameter(self.num_chars, self.num_chars)
        self.w2 = nn.Parameter(self.num_chars, self.num_chars)
        self.w3 = nn.Parameter(self.num_chars, len(self.languages))
    ...
    def run(self, xs):
        ...
        for i in range(1, len(xs)):
            x = nn.Add(nn.Linear(xs[i], self.w1), nn.Linear(x, self.w2))
            x = nn.ReLU(x)
    ...
    def train(self, dataset):
        ...
        for x, y in dataset.iterate_forever(batch_size):
            loss = self.get_loss(x, y)
            g_w1, g_w2, g_w3 = nn.gradients(loss, [self.w1, self.w2, self.w3])
            self.w1.update(g_w1, -learning_rate)
            self.w2.update(g_w2, -learning_rate)
            self.w3.update(g_w3, -learning_rate)
